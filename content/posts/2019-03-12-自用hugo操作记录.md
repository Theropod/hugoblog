...
- 关于目录，在上面的模板页面里合适位置加入
    ...
    {{ end }}
    ```
    此为官网上的例子，非常容易理解。WordCount这个判断要是不设置hasCJKlanguage会统计不出来。
- Disqus模板Hugo已经内置，申请一个Disqus Site得到shortname，作为`DisqusShortname`放在config.toml，在single.html里面加入教程里的一行引用就ok。
- Google和Baidu的Analytics，注册服务得到跟踪ID之后加入网站设置就ok
